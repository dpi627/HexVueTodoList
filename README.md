...
- `import.meta.env.BASE_URL`：應用的基礎 URL（base path），來自 `vite.config.js` 中的 `base` 配置。若未設置，預設為 `'/'`。
- `import.meta.env.MODE`：當前運行模式，例如 `'development'` 或 `'production'`。
- `import.meta.env.DEV`：布林值，表示是否為開發模式（`true` 表示開發，`false` 表示生產）。
- `import.meta.env.PROD`：布林值，表示是否為生產模式（`true` 表示生產，`false` 表示開發）。
- `import.meta.env.SSR`：布林值，表示是否為服務器端渲染模式。
